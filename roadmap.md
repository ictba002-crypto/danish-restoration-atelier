# Danish Modern L.A. Redesign Roadmap

- [x] Generate homepage + PDP imagery
- [x] Build design system (tokens, fonts, components)
- [x] Build homepage sections
- [x] Build product detail page
- [x] Wire shared header/footer into root layout
- [x] Show a signature piece image beside each designer name (instead of portraits)
- [x] Expand designer grid to all 8 designers from reference
- [x] Verify build and preview
- [x] Update PDP with one full-width Add to Cart row, a secondary action row, and four requested proof points
